function [M, chi_history] = build_monte_carlo_nearest(W, nSteps, M_explicit, doAnimate)

    w = W(:);
    N = numel(w);              % total number of states
    ...
    for step = 1:nSteps
        ...
        % chose one of the candidates
        proposal = neigh(randi(numel(neigh)));
